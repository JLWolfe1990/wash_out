...
xml.definitions 'xmlns' => 'http://schemas.xmlsoap.org/wsdl/',
  ...
  xml.portType :name => "#{@name}_port" do
    @map.each do |operation, formats|
      xml.operation :name => operation do
        xml.input :message => "#{@prefix}:#{operation}"
        xml.output :message => "#{@prefix}:#{formats[:response_tag]}"
      end
    end
  end

  xml.binding :name => "#{@name}_binding", :type => "#{@prefix}:#{@name}_port" do
  ...
  xml.service :name => "service" do
    xml.port :name => "#{@name}_port", :binding => "#{@prefix}:#{@name}_binding" do
